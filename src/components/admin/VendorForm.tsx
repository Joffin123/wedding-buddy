    ...
    <form action={action} className="max-w-2xl space-y-5 rounded-2xl border border-purple-100 bg-white p-6 shadow-sm">
      <div className="grid gap-4 sm:grid-cols-2">
        ...
        <div>
          ...
          <input id="slug" name="slug" required defaultValue={vendor?.slug} placeholder="lens-and-light-studio" className={inputClass} />
        </div>
      </div>
      ...
      <div className="grid gap-4 sm:grid-cols-2">
        <div>
          ...
          <select id="category" name="category" required defaultValue={vendor?.category ?? CATEGORIES[0]} className={inputClass}>
            {CATEGORIES.map((c) => <option key={c} value={c}>{c}</option>)}
          </select>
        </div>
        <div>
          <label className={labelClass} htmlFor="location">Location</label>
          <input id="location" name="location" required defaultValue={vendor?.location} placeholder="Kochi" className={inputClass} />
        </div>
      </div>

      <div>
        <label className={labelClass} htmlFor="tagline">Tagline</label>
        <input id="tagline" name="tagline" required defaultValue={vendor?.tagline} className={inputClass} />
      </div>

      <div className="grid gap-4 sm:grid-cols-3">
        <div>
          <label className={labelClass} htmlFor="price_from">Starting price (₹)</label>
          <input id="price_from" name="price_from" type="number" min="0" step="500" required defaultValue={vendor?.price_from} className={inputClass} />
        </div>
        <div>
          <label className={labelClass} htmlFor="price_unit">Price unit</label>
          <input id="price_unit" name="price_unit" defaultValue={vendor?.price_unit} placeholder="per wedding / per plate" className={inputClass} />
        </div>
        <div>
          <label className={labelClass} htmlFor="initials">Initials (fallback avatar)</label>
          <input id="initials" name="initials" maxLength={3} defaultValue={vendor?.initials} placeholder="LL" className={inputClass} />
        </div>
      </div>
      ...
      <div className="grid gap-4 sm:grid-cols-2">
        <div>
          ...
          <input id="rating" name="rating" type="number" min="0" max="5" step="0.1" defaultValue={vendor?.rating} className={inputClass} />
        </div>
        <div>
          <label className={labelClass} htmlFor="reviews">Review count</label>
          <input id="reviews" name="reviews" type="number" min="0" defaultValue={vendor?.reviews} className={inputClass} />
        </div>
      </div>
      ...
      <div>
        ...
        <input id="gradient" name="gradient" defaultValue={vendor?.gradient} placeholder="from-purple-200 via-pink-100 to-indigo-200" className={inputClass} />
      </div>

      <div>
        <label className={labelClass} htmlFor="image">Photo {vendor ? "(leave empty to keep current)" : ""}</label>
        {currentImage && (
          <div className="relative mt-2 h-24 w-24 overflow-hidden rounded-xl border border-purple-100">
      ...
      <div className="flex gap-6">
        <label className="flex items-center gap-2 text-sm text-[#374151]">
          <input type="checkbox" name="verified" defaultChecked={vendor?.verified} className="h-4 w-4 rounded border-purple-300 text-[#8B31C7] focus:ring-[#8B31C7]" />
          Verified
        </label>
        <label className="flex items-center gap-2 text-sm text-[#374151]">
          <input type="checkbox" name="featured" defaultChecked={vendor?.featured} className="h-4 w-4 rounded border-purple-300 text-[#8B31C7] focus:ring-[#8B31C7]" />
          Featured
        </label>
      </div>
