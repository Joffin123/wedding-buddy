...
const KERALA_DISTRICTS = [
  "Kochi", "Kumarakom", "Munnar", "Thekkady", "Varkala", "Wayanad",
  "Thiruvananthapuram", "Kollam", "Alappuzha", "Thrissur",
  "Palakkad", "Kozhikode", "Kannur", "Kasaragod",
];
...
type WeddingContext = { crowd: string; place: string; foodType: string };
...
function LanguageToggle({ language, onChange }: { language: "en" | "ml"; onChange: (l: "en" | "ml") => void }) {
  return (
    <div className="flex items-center gap-1 rounded-full border border-rose-200 bg-rose-50/80 p-0.5 text-[11px] font-bold">
      ...
      <button
        onClick={() => onChange("ml")}
        className={`rounded-full px-2.5 py-1 transition-all ${language === "ml" ? "bg-white shadow-sm text-[#2A1A33]" : "text-[#2A1A33]/50 hover:text-[#2A1A33]/80"}`}
      >
        മല
      </button>
    </div>
...
function OnboardingWizard({ onComplete }: { onComplete: (data: WeddingContext) => void }) {
  ...
  function finish(food: string) {
    onComplete({ crowd, place, foodType: food });
  }
      ...
      {items.map((v, i) => (
        ...
        >
          <div className={`relative aspect-[5/3] overflow-hidden bg-gradient-to-br ${v.gradient}`}>
            {v.image ? (
              <Image src={v.image} alt={v.name} fill sizes="(max-width:640px) 100vw, 300px" className="object-cover transition-transform duration-700 group-hover:scale-105" />
            ) : (
          ...
          <div className={`relative aspect-[4/3] overflow-hidden bg-gradient-to-br ${v.gradient}`}>
            {v.image ? (
              ...
              <div className="absolute inset-0 flex items-center justify-center">
                <span className="font-display text-6xl font-medium text-white/80 drop-shadow-sm">{v.initials}</span>
              </div>
            )}
            <div className="absolute inset-x-0 top-0 flex items-start justify-between p-2.5">
              {v.verified && <span className="rounded-full border border-rose-200 bg-white/95 px-2 py-0.5 text-[10px] font-bold uppercase tracking-[0.14em] text-[#2A1A33] backdrop-blur-sm">✓ Verified</span>}
              {v.featured && <span className="ml-auto rounded-full bg-gradient-to-r from-amber-400 to-rose-400 px-2 py-0.5 text-[10px] font-bold uppercase tracking-[0.14em] text-white shadow-md">★ Top</span>}
            </div>
          </div>
          <div className="p-3.5">
            <div className="flex items-start justify-between">
              <h4 className="font-display text-lg font-medium leading-tight text-[#2A1A33]">{v.name}</h4>
              <span className="ml-2 flex-shrink-0 text-xs font-semibold text-amber-600">★ {v.rating.toFixed(1)}</span>
            </div>
      ...
      {items.map((g, i) => (
        ...
        >
          ...
          <figcaption className="p-2.5">
            ...
            {g.caption && <p className="mt-0.5 line-clamp-2 text-[10px] leading-relaxed text-[#2A1A33]/60">{g.caption}</p>}
          </figcaption>
        </motion.figure>
      ))}
    </div>
...
function GradientFallback({ label }: { label: string }) {
  return (
    <div className="absolute inset-0 flex items-center justify-center">
      ...
      <span className="absolute bottom-2 right-3 text-[10px] font-semibold uppercase tracking-[0.18em] text-white/90 drop-shadow">{label}</span>
    </div>
  );
}
...
export default function InlineChat() {
  ...
  // Use a ref so the transport function always reads the latest language/context
  // without recreating the transport instance on every render.
  const bodyRef = useRef<{ language: "en" | "ml"; weddingContext: WeddingContext | null }>({
    language: "en",
    weddingContext: null,
  });
  bodyRef.current = { language, weddingContext };

  const transportRef = useRef(
    new DefaultChatTransport({ body: () => bodyRef.current })
  );

  const { messages, sendMessage, status, error, stop, clearError } = useChat({
    transport: transportRef.current,
    onError: (err) => console.error("[InlineChat] error:", err),
  });
  ...
  function handleOnboardingComplete(data: WeddingContext) {
    setWeddingContext(data);
              ...
              >
                {messages.length === 0 && (
                  <div className="mx-auto max-w-2xl space-y-5">
                    ...
                    <div>
                      <p className="mb-3 text-[10px] font-semibold uppercase tracking-[0.22em] text-[#2A1A33]/50">Try asking</p>
                    ...
                    return (
                      ...
                      >
                        <div className={`max-w-[92%] ${isUser ? "" : "w-full"}`}>
                          ...
                          {!isUser && (m.parts as unknown as ToolPart[])
              ...
              >
                <div className="flex items-center gap-2 rounded-full border border-rose-200 bg-white pl-5 pr-1.5 focus-within:border-rose-400 focus-within:shadow-[0_0_0_4px_rgba(232,115,155,0.12)] transition-all">
                  <input
                    ...
                    onKeyDown={onKeyDown}
                    placeholder={isBusy ? "Concierge is thinking…" : language === "ml" ? "ഇവിടെ ടൈപ്പ് ചെയ്യൂ…" : "Ask anything about your wedding…"}
                    autoComplete="off"
                    spellCheck={false}
